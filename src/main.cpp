...
uint256 hashGenesisBlock("0x00000fb7da361bd7c8ea7d6c6c500e13f70e6710edda526c973106e06412a750");

static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // CvvCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "CvvCoin Signed Message:\n";
...
{
    // CvvCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
// Launch specs
static const int64 nTargetTimespan = 24 * 60 *60; // CvvCoin: each block
static const int64 nTargetSpacing = 60; // CvvCoin: 60 seconds
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // CvvCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, CvvCoin - DarkGravity, written by Evan Duffield - evan@CvvCoin.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // CvvCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // CvvCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x0000053d47b7b30f68c33ec273d92e1c33d78a8c30d70d2385f1b16993e67a16");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
      ...
      //vMerkleTree: 3f66108684400d84e2ffb528360d534fa19ecaf47c7c4c2ca1d1352ec4a351ad
        ...
        // Genesis block
        const char* pszTimestamp = "Iranian Gov’t Authorizes Cryptocurrency Mining As Industrial Activity";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("043f758a228a62dbfe11e3f4508ced41ba5bd3f32a0db5e277061d9cff2049c108730b96dd8cdcf20224a945484c5984558577f5ff9e367bc7a20845575819069a") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1564390547;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 914627;

        if (fTestNet)
        {
            block.nTime    = 1564390554;
            block.nNonce   = 1403359;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xb1f854b122e6294d052d8ebe3945de5f7f0b0f28d9bb92ea38178cc0c0e9c064"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0x4d, 0x43, 0x56, 0x56 };  // MCVV
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CvvCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CvvCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CvvCoinMiner(CWallet *pwallet)
{
    printf("CvvCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("CvvCoin-miner");
    ...
    try { while (true) {
        ...
        printf("Running CvvCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CvvCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CvvCoinMiner, pwallet));
}
